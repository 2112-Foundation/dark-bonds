...
pub fn buy_swap(ctx: Context<BuySwap>) -> Result<()> {
    ...
    let main: &mut Account<Main> = &mut accounts.main;
    ...
    // Now, adjust the bond main's cut so that the total exactly matches the bond price.
    ...
    // Transfer liquidity coin cut to the ATA of the main admin
    token::transfer(
